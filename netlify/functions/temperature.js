...
exports.handler = async function (event) {
  ...
  const [liveResult, maxMinResult, metarResult, rhrreadResult, webResult] = await Promise.allSettled([fetchLive(), fetchMaxMin(), fetchMetar(), fetchRhrread()]);
  ...
  // 仲有嘢俾得到就200,四條全死先502。
  const ok = !!(response.live || response.today || response.metars || response.rain || response.web);
  return {
